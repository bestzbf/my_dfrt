...
def resolve_patch_data_config(args):
    if args.disable_precompute_local_patches and args.patch_provider in {
        "precomputed_resized",
        "precomputed_highres",
    }:
        raise ValueError(
            f"patch_provider='{args.patch_provider}' requires local patch precomputation, "
...
def _drop_confidence_head_weights(state_dict, is_main=False, context="checkpoint"):
    keys_to_delete = [key for key in state_dict.keys() if "head_conf" in key]
    for key in keys_to_delete:
        if is_main:
            print(f"Dropping incompatible {context} weight: {key}")
        del state_dict[key]
...
def load_checkpoint(
    ...
    *,
    ...
    if drop_conf_head:
        _drop_confidence_head_weights(model_state, is_main=is_main, context="checkpoint")

    missing, unexpected = target_model.load_state_dict(model_state, strict=False)
    ...
    if not model_only:
        if missing or unexpected:
            raise ValueError(
                "Full checkpoint resume requires an exact model match, but missing/unexpected keys were found. "
                "Use --resume-model-only for partial loading or --pretrained-weights for transfer initialization."
            )
        if drop_conf_head:
            raise ValueError(
                "--drop-conf-head-on-load changes model weights, so it cannot be combined with full-state resume. "
                "Use --resume-model-only instead."
            )
        if optimizer is None or scheduler is None:
            raise ValueError("Full checkpoint resume requires optimizer and scheduler instances")

        optimizer_state = checkpoint.get("optimizer_state_dict")
        scheduler_state = checkpoint.get("scheduler_state_dict")
        scaler_state = checkpoint.get("scaler_state_dict")
        if optimizer_state is None or scheduler_state is None:
            raise ValueError("Checkpoint is missing optimizer/scheduler state required for full resume")
        ...
        if is_main:
            print("Restored optimizer, scheduler, and scaler state from checkpoint.")
    elif is_main:
        print("WARNING: Loading model weights only; optimizer, scheduler, and scaler states were skipped.")
...
def main():
    ...
    if is_main:
        ...
        print(f"Resume mode: {'model-only' if args.resume_model_only else 'full-state'}")
        print(f"DataLoader workers: {args.num_workers}")
        if args.num_workers > 0:
            print(f"DataLoader prefetch factor: {args.prefetch_factor}")
        print(
            "Precompute local patches on CPU: "
            f"{patch_data_config['precompute_local_patches']}"
        )
        if patch_data_config["precompute_local_patches"]:
            print(f"Local patch source: {patch_data_config['local_patch_source']}")
        print("=" * 60)
    ...
    for epoch_idx in range(start_epoch, max_epochs):
        ...
        for batch in train_loader:
            ...
            if not losses:
                train_compute_total += time.time() - step_compute_start
                batch_wait_start = time.time()
                continue
        ...
        if is_main and completed_epoch % max(1, args.log_epochs) == 0:
            print(
                ...
                f"DataWait: {train_data_wait_total / max(1, batch_fetch_count):.2f}s/batch | "
                f"Compute: {train_compute_total / max(1, batch_fetch_count):.2f}s/batch | "
