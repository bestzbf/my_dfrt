...
def load_checkpoint(checkpoint_path, model, optimizer=None, scheduler=None, scaler=None):
    ...
    # [CRITICAL FIX] 
    # Because we completely changed the mathematical meaning of the confidence head 
    # (from outputting 'c' to outputting 's = -log(c)'), the old weights for head_conf
    # are completely incompatible with the new loss landscape.
    # We MUST reset the weights of head_conf to random initialization so it doesn't 
    # start with a massive built-in bias that destroys the network.
    keys_to_delete = [k for k in model_state.keys() if "head_conf" in k]
    for k in keys_to_delete:
        print(f"Dropping incompatible weight: {k}")
        del model_state[k]
    ...
    # DO NOT load optimizer/scheduler states when making architectural/loss changes!
    # The old Adam momentum contains massive gradients for the old loss landscape.
    # If we load them, the momentum will blow up the new weights.
    print("WARNING: Skipping optimizer and scheduler state loading due to loss landscape shift.")
...
def load_pretrained_weights(checkpoint_path, model, is_main=False):
    ...
    # [CRITICAL FIX] Drop head_conf weights
    keys_to_delete = [k for k in state_dict.keys() if "head_conf" in k]
    for k in keys_to_delete:
        if is_main:
            print(f"Dropping incompatible pretrained weight: {k}")
        del state_dict[k]
