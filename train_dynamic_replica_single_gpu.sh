...
EPOCHS="${EPOCHS:-100}"
LR="${LR:-5e-5}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/dynamic_replica_single_gpu_4}"
...
echo "[dynamic_replica] CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "[dynamic_replica] OUTPUT_DIR=$OUTPUT_DIR"
echo "[dynamic_replica] NUM_WORKERS=$NUM_WORKERS"
echo "[dynamic_replica] PATCH_PROVIDER=$PATCH_PROVIDER"
if [[ -n "$PRETRAIN" ]]; then
  if [[ ! -f "$PRETRAIN" ]]; then
    echo "[dynamic_replica] PRETRAIN not found: $PRETRAIN" >&2
    exit 1
  fi
  echo "[dynamic_replica] PRETRAIN=$PRETRAIN"
else
  echo "[dynamic_replica] PRETRAIN=<none>"
fi
