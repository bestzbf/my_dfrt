...
def compute_confidence_loss(
    ...
    """Compute the confidence penalty term ``-log(confidence)``.

    Args:
        confidence: (B, N, 1) positive confidence weights
        mask: (B, N) optional validity mask
    ...
    # Only positivity is required by the loss; values above 1 remain valid.
    confidence = torch.clamp(confidence, min=1e-8)
...
class D4RTLoss(nn.Module):
    ...
    def _reduce_point_loss(
        ...
        """Apply optional confidence weighting and masked mean reduction."""
        if self.use_confidence_weighting:
            confidence_weight = torch.lerp(
                torch.ones_like(confidence),
                confidence,
                self.confidence_weighting_factor,
            )
    ...
    def set_confidence_schedule(self, lambda_conf: float, weighting_factor: Optional[float] = None) -> None:
        ...
        if weighting_factor is None:
            weighting_factor = 1.0 if self.lambda_conf > 0.0 else 0.0
        weighting_factor = max(0.0, min(1.0, float(weighting_factor)))
        self.confidence_weighting_factor = weighting_factor
        self.use_confidence_weighting = weighting_factor > 0.0
