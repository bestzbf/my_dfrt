...
def _resize_normal(normal: np.ndarray, h: int, w: int) -> np.ndarray:
    ...
    safe = norm > 1e-6
    resized = np.where(safe, resized / np.where(safe, norm, 1.0), resized)
...
def _sample_center_crop(height: int, width: int) -> CropParams:
    """Deterministic center square crop (for validation / overfit mode)."""
    crop_size = min(height, width)
    x0 = (width - crop_size) // 2
    y0 = (height - crop_size) // 2
    return CropParams(x0=x0, y0=y0, crop_w=crop_size, crop_h=crop_size)
...
def _apply_color_aug(
    ...
    hue_delta   = rng.uniform(-0.1, 0.1) * 180.0
    ...
    for frame in rgb_frames:
        img = np.clip(frame * brightness, 0.0, 1.0)
        ...
        if apply_blur:
            img = cv2.GaussianBlur(img, ksize=(5, 5), sigmaX=blur_sigma, sigmaY=blur_sigma)
...
class GeometryTransformPipeline:
    ...
    Args:
        ...
        use_augs:   If True, apply random crop and colour augmentation.
                    If False, use deterministic centre crop and no colour aug.
    """
    ...
    def __call__(
        ...
        # ------------------------------------------------------------------ #
        cropped_images = [img[y0:y0 + ch, x0:x0 + cw] for img in clip.images]
