...
4.  Depth content: float32, finite, depth mask applied (no negative values).
5.  Intrinsics: shape (T,3,3), finite, positive focal lengths, principal point
    near image centre (≤ 15% of half-dimension offset).
6.  Extrinsics: shape (T,4,4), finite, rotation near-orthonormal, det(R)≈+1,
    last row == [0,0,0,1].
7.  Reprojection cross-check: for frames that share depth, unproject a batch
    of depth pixels to world space with E_t^{-1} and re-project onto the same
    frame with K_t and E_t.  Error must be < 1 px.
8.  Depth–intrinsics consistency: verify cx≈W/2 and cy≈H/2 (pp ≈ (0,0) in NDC).
9.  Boundary frame indices: first and last frame load without error.
10. Error-handling: out-of-range index, empty list, unknown sequence name.
11. Per-sequence adapter.sanity_check() return structure and result.
12. Multi-category coverage: at least one sequence per requested category.

Usage
-----
# Quick: 2 categories, 3 sequences each
python check_co3dv2.py --data-root /data2/d4rt/datasets/Co3Dv2 \\
    --categories apple chair --max-seqs 3

# Single sequence
python check_co3dv2.py --data-root /data2/d4rt/datasets/Co3Dv2 \\
    --categories apple --sequence apple/110_13048_23163

# Full scan of selected categories
python check_co3dv2.py --data-root /data2/d4rt/datasets/Co3Dv2 \\
    --categories apple book --max-seqs 0 --verbose

# All 51 categories, 1 sequence each (comprehensive coverage check)
python check_co3dv2.py --data-root /data2/d4rt/datasets/Co3Dv2 \\
    --max-seqs 1
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Root directory of the Co3Dv2 dataset.",
    )
    p.add_argument(
        "--categories", nargs="+", default=None,
        metavar="CAT",
        help="Categories to test.  Default: all 51.",
    )
    p.add_argument(
        "--subset-name", default="fewview_train",
        choices=["fewview_train", "fewview_dev", "fewview_test",
                 "manyview_dev_0", "manyview_dev_1", "manyview_test_0"],
        help="Co3D subset to use.",
    )
    p.add_argument(
        "--split", default="train",
        choices=["train", "val", "test"],
        help="Split key within the subset file.",
    )
    p.add_argument(
        "--sequence", default=None,
        help="Test a single sequence ('<category>/<seq_name>').  "
             "Overrides --max-seqs.",
    )
    p.add_argument(
        "--max-seqs", type=int, default=2,
        help="Max sequences to test per run (0 = all).",
    )
    p.add_argument(
        "--clip-len", type=int, default=6,
    ...
    p.add_argument(
        "--reproj-max-frames", type=int, default=3,
...
def check_schema(c: _Checker, clip: UnifiedClip, T: int) -> None:
    c.require(clip.dataset_name == "co3dv2",
              f"dataset_name='{clip.dataset_name}'=='co3dv2'")
    ...
    if isinstance(clip.metadata, dict):
        ...
        c.require("category" in clip.metadata, "metadata.category present")
...
def check_depth_content(c: _Checker, clip: UnifiedClip) -> None:
    for t, dep in enumerate(clip.depths):
        ...
        c.require((dep >= 0).all(), f"depths[{t}] no negative values (mask applied)")
        n_valid = int((dep > 0).sum())
        if n_valid == 0:
            c.warn(f"depths[{t}] has NO positive pixels (depth mask may be all-invalid)")
        elif c.verbose:
            total = dep.size
            valid_dep = dep[dep > 0]
            c.notes.append(
                f"  info depths[{t}]: min={valid_dep.min():.4f}, "
                f"max={valid_dep.max():.4f}, coverage={n_valid/total:.1%}"
            )
...
def check_intrinsics(c: _Checker, clip: UnifiedClip) -> None:
    ...
    # principal point near image centre (within 15% of half-dim)
    if clip.images:
        ...
        tol_y = H * 0.15
        c.require(
            np.all(np.abs(cx - W / 2.0) <= tol_x)
            and np.all(np.abs(cy - H / 2.0) <= tol_y),
            f"principal point near centre: "
            f"cx∈[{cx.min():.1f},{cx.max():.1f}] ≈ W/2={W/2:.1f}, "
            f"cy∈[{cy.min():.1f},{cy.max():.1f}] ≈ H/2={H/2:.1f}",
        )
...
def check_error_handling(c: _Checker, adapter: Co3Dv2Adapter, seq_name: str) -> None:
    ...
    raised = False
    try:
        adapter.get_sequence_info("__bad_category__/__bad_seq__")
...
def main() -> None:
    ...
    print(f"\n{'='*60}")
    print(f"  Co3Dv2 adapter check")
    print(f"  subset={args.subset_name!r}, split={args.split!r}")
    ...
    if args.sequence:
        ...
        # Round-robin across categories so every category gets at least one
        # sequence, then fill remaining slots evenly.
        from collections import defaultdict
        cat_to_seqs: dict[str, list[str]] = defaultdict(list)
        for s in all_seqs:
            cat_to_seqs[s.split("/")[0]].append(s)

        seqs_to_check: list[str] = []
        # First pass: one per category (up to max_seqs)
        for seqs in cat_to_seqs.values():
            if len(seqs_to_check) >= args.max_seqs:
                break
            seqs_to_check.append(seqs[0])
        # Second pass: fill remaining slots with evenly-spaced sequences
        if len(seqs_to_check) < args.max_seqs:
            remaining = args.max_seqs - len(seqs_to_check)
            already = set(seqs_to_check)
            rest = [s for s in all_seqs if s not in already]
            step = max(1, len(rest) // remaining)
            seqs_to_check += rest[::step][:remaining]
    ...
    # ── Multi-category coverage check ────────────────────────────────
    if len(categories) > 1 and not args.sequence:
        cats_covered = {s.split("/")[0] for s in seqs_to_check}
        cats_missing = set(categories) - cats_covered
        if cats_missing:
            print(
                f"  [warn] categories not covered by sampled sequences: "
                f"{sorted(cats_missing)}"
            )
        else:
            if args.verbose:
                print(f"  all {len(cats_covered)} categories represented")
