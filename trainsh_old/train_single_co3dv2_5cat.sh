...
CONFIG="${CONFIG:-configs/single_co3dv2_5cat.yaml}"
VAL_CONFIG="${VAL_CONFIG:-configs/single_co3dv2_5cat_val.yaml}"
BATCH_SIZE="${BATCH_SIZE:-10}"
NUM_WORKERS="${NUM_WORKERS:-8}"
GRAD_ACCUM="${GRAD_ACCUM:-1}"
EPOCHS="${EPOCHS:-200}"
LR="${LR:-1e-4}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/single_co3dv2_5cat}"
...
echo "[single_co3dv2_5cat] CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "[single_co3dv2_5cat] OUTPUT_DIR=$OUTPUT_DIR"
echo "[single_co3dv2_5cat] categories: vase bench toaster orange apple"
if [[ -n "$PRETRAIN" ]]; then
  echo "[single_co3dv2_5cat] PRETRAIN=$PRETRAIN"
fi
