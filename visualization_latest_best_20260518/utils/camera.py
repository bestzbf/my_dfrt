...
def _umeyama_pytorch3d(
    ...
    Convention: target = s * R @ source + t

    PyTorch3D's corresponding_points_alignment uses the same convention as our
    native implementation, so results are directly compatible.
    """
    ...
    # Convention: X_target = s * R @ X_source + T
...
def _umeyama_native(
    ...
    This implements the standard Umeyama algorithm (TPAMI 1991) using column vectors.
    """
    ...
    # Compute translation: t = mu_target - s * R @ mu_source
...
def umeyama_ransac(
    ...
    """RANSAC-based rigid body alignment using Umeyama on minimal samples.

    Args:
        source: (N, 3) source points
        target: (N, 3) target points
        weights: (N,) optional confidence weights for sampling bias and final fit
        inlier_thresh: residual threshold for inlier classification
    ...
    Returns:
        R: (3, 3) rotation matrix
        t: (3,) translation vector
        s: scalar scale (always 1.0 since with_scale=False)
        inlier_mask: (N,) boolean mask of inliers from best hypothesis
    """
    N = source.shape[0]
    ...
    if N < min_inliers:
        return (
            torch.eye(3, device=device, dtype=source.dtype),
            torch.zeros(3, device=device, dtype=source.dtype),
            torch.ones(1, device=device, dtype=source.dtype),
            torch.zeros(N, device=device, dtype=torch.bool),
        )

    # Sampling probabilities biased by confidence
    if weights is not None and weights.sum() > 0:
        probs = weights / weights.sum()
    else:
        probs = None

    # Adaptive threshold: use percentile of pairwise distances if fixed thresh too small
    pts_scale = torch.cdist(source[:min(50, N)], source[:min(50, N)]).median().clamp(min=1e-6)
    adaptive_thresh = max(inlier_thresh, float(pts_scale) * 0.05)

    best_inlier_count = 0
    best_inliers = torch.zeros(N, device=device, dtype=torch.bool)
    ...
    for _ in range(max_iters):
        ...
        try:
            R_hyp, t_hyp, _ = umeyama_alignment(
                source[idx], target[idx], with_scale=False
            )
        except Exception:
            continue

        residuals = (source @ R_hyp.T + t_hyp - target).norm(dim=-1)
        inliers = residuals < adaptive_thresh
    ...
    # Final weighted fit on inliers
    w_final = weights[best_inliers] if weights is not None else None
    R, t, s = umeyama_alignment(
        source[best_inliers], target[best_inliers], w_final, with_scale=False
    )
    return R, t, s, best_inliers
...
def estimate_camera_pose(
    ...
    Args:
        ...
        sample_margin: Margin from edges (0.0=full image, 0.1=avoid 10% edges)
    ...
    # Create grid of query points (configurable edge margin)
    u = torch.linspace(sample_margin, 1.0 - sample_margin, w, device=device)
    v = torch.linspace(sample_margin, 1.0 - sample_margin, h, device=device)
    ...
    # Rigid alignment (paper main text uses rigid body transform, not Sim(3)).
    # points_i = R @ points_j + t
    R, t, _ = umeyama_alignment(points_j_valid, points_i_valid, weights_valid, with_scale=False)
...
def sim3_alignment(
    ...
    NOTE: This is an *eval-time* alignment utility (used for trajectory
    evaluation, e.g. ATE).  It is NOT the paper's pairwise camera pose solver.
    The paper main text uses a rigid body transform (SE(3)); see
    estimate_camera_pose() which calls umeyama_alignment with_scale=False.
    Sim(3) here is appropriate for long-video evaluation as noted in the
    appendix.
    ...
    # Umeyama alignment with scale (Sim(3)) — eval-time only
