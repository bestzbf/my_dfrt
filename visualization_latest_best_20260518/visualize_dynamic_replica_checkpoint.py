...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--camera-pose-ransac-iters", type=int, default=256, help="相机位姿 RANSAC 迭代次数。")
    parser.add_argument("--camera-pose-inlier-thresh", type=float, default=0.1, help="相机位姿 RANSAC 内点阈值。")
...
def resolve_model_variant(requested_variant: str, state: dict[str, torch.Tensor]) -> str:
    ...
    print(f"[vis] Auto-detected model_variant={inferred_variant} from checkpoint tensor shapes", flush=True)
...
def apply_sim3_to_poses(pred_poses: torch.Tensor, gt_poses: torch.Tensor) -> torch.Tensor:
    ...
    # sim3_alignment maps predicted world coordinates into GT world coordinates:
    # x_gt = scale * R * x_pred + t.  A camera-to-world orientation must be
    # left-multiplied by the same global rotation.
    aligned[:, :3, :3] = rotation @ pred_poses[:, :3, :3]
...
def visualize_camera_parameters(
    ...
    try:
        gt_c2w, pred_c2w, pose_metrics = estimate_camera_trajectory_for_result(
            ...
            device=device,
            patch_provider=args.patch_provider,
            ransac_iters=args.camera_pose_ransac_iters,
            inlier_thresh=args.camera_pose_inlier_thresh,
        )
