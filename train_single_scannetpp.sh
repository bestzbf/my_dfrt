...
CONFIG="${CONFIG:-configs/single_scannetpp.yaml}"
VAL_CONFIG="${VAL_CONFIG:-configs/single_scannetpp_val.yaml}"
BATCH_SIZE="${BATCH_SIZE:-10}"
NUM_WORKERS="${NUM_WORKERS:-8}"
GRAD_ACCUM="${GRAD_ACCUM:-1}"
EPOCHS="${EPOCHS:-200}"
LR="${LR:-1e-4}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/single_scannetpp}"
...
echo "[single_scannetpp] CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "[single_scannetpp] CONFIG=$CONFIG"
echo "[single_scannetpp] OUTPUT_DIR=$OUTPUT_DIR"
echo "[single_scannetpp] PATCH_PROVIDER=$PATCH_PROVIDER"
echo "[single_scannetpp] RESET_CONF_HEAD_ON_PRETRAIN=$RESET_CONF_HEAD_ON_PRETRAIN"
if [[ -n "$PRETRAIN" ]]; then
  if [[ ! -f "$PRETRAIN" ]]; then
    echo "[single_scannetpp] PRETRAIN not found: $PRETRAIN" >&2
    exit 1
  fi
  echo "[single_scannetpp] PRETRAIN=$PRETRAIN"
fi
echo "[single_scannetpp] VARIANT=$VARIANT"
